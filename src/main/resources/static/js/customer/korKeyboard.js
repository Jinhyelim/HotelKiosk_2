;(function () {
    ...
    // 자음, 모음 배열
    const doubleConsonants = ['ㅃ', 'ㅉ', 'ㄸ', 'ㄲ', 'ㅆ'];
    const layout = [
        ['ㅂ', 'ㅈ', 'ㄷ', 'ㄱ', 'ㅅ', 'ㅛ', 'ㅕ', 'ㅑ', 'ㅐ', 'ㅔ'],
        ['ㅁ', 'ㄴ', 'ㅇ', 'ㄹ', 'ㅎ', 'ㅗ', 'ㅓ', 'ㅏ', 'ㅣ', 'ㅖ'],
        ['ㅋ', 'ㅌ', 'ㅊ', 'ㅍ', 'ㅠ', 'ㅜ', 'ㅡ']
    ...
    let row1Html = `
        <div class="kb-row">
            ...
            ${doubleConsonants.map(k => `<button class="kb-key" data-k="${k}">${k}</button>`).join('')}
            <button class="kb-key kb-func" data-a="del">⌫</button>
        </div>
    `;

    // 2. 중간 자모음 줄
    let middleRowsHtml = layout.map(row => {
    ...
    let lastRowHtml = `
        <div class="kb-row">
            <button class="kb-key kb-confirm" data-a="confirm" style="flex: 1;">확 인</button>
        </div>
    `;
