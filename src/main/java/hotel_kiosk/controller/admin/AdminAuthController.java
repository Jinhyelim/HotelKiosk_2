...
public class AdminAuthController {
    ...
    public ResponseEntity<Map<String, Object>> loginStep1(
        ...
        String adminId = body.get("username");
        String password = body.get("password");

        try {
            // 1. 관리자 정보 조회
            AdminDTO admin = adminService.getOne(adminId);

            // 2. 비밀번호 일치 확인
            if (admin != null && passwordEncoder.matches(password, admin.getPassword())) {

                // 3. 인증 코드 발송
                emailService.sendAuthCode(admin.getAdminEmail());

                // 4. Redis에 저장된 인증번호를 로그로 출력
                String generatedCode = emailAuthService.getAuthCode(admin.getAdminEmail());
                log.info("=================================================");
                log.info("테스트용 인증번호 확인 (ID: {}): {}", adminId, generatedCode);
                log.info("=================================================");

                // 5. 세션에 정보 저장 (2차 인증 시 사용)
                session.setAttribute("PENDING_ADMIN_ID", adminId);
                session.setAttribute("PENDING_ADMIN_EMAIL", admin.getAdminEmail());
            ...
            log.error("Login Step1 Error", e);
        }

        return ResponseEntity.ok(Map.of("success", false, "message", "아이디 또는 비밀번호가 틀립니다."));
    }

    @PostMapping("/admin/login/step2")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> loginStep2(
            @RequestBody Map<String, String> body,
            HttpSession session
    ) {
        String email = (String) session.getAttribute("PENDING_ADMIN_EMAIL");
        String code = body.get("code");

        try {
            // 기존 서비스의 verifyCode 호출
            emailAuthService.verifyCode(email, code);

            // 인증 성공 시 세션 최종 승인 등 후속 처리
            session.setAttribute("ADMIN_LOGIN_SUCCESS", true);
