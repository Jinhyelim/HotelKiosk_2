...
import hotel_kiosk.dto.customer.OptionItemDTO;
import hotel_kiosk.exception.DuplicateReservationException;
...
public class OnSiteReservationService {
    ...
    @Transactional
    public List<String> preRegisterReservation(Members members, Reservations reservations, List<Integer> roomNos, List<OptionItemDTO> selectedOptions) {
        ...
        for (Integer roomNo : roomNos) {
            // 중복 예약 체크
            int duplicateCount = onSiteReservationMapper.countDuplicateReservation(roomNo, reservations.getCheckinDate(), reservations.getCheckoutDate());
            if (duplicateCount > 0) {
                throw new DuplicateReservationException(roomNo);
            }
            ...
            // 6. 해당 객실의 옵션 내역 options_record에 INSERT
            if (selectedOptions != null) {
                for (OptionItemDTO opt : selectedOptions) {
                    if (opt.getRoomNo() == roomNo && opt.getQuantity() > 0) {
                        onSiteReservationMapper.insertOptionsRecord(
                                res.getIdx(), opt.getOptionId(), opt.getQuantity(), opt.getOptionCharge()
                        );
                    }
                }
            }
