...
public class ReservationReadServiceImpl implements ReservationReadService {
    ...
    @Override
    public List<ReservationsDTO> readByPhone(String phone) {
        List<Reservations> membersList = reservationReadMapper.findReservationByPhone(phone);
        List<ReservationsDTO> dtoList = new ArrayList<>();
        for (Reservations reservations : membersList) {
            dtoList.add(modelMapper.map(reservations, ReservationsDTO.class));
        }
        return dtoList;
    }
    ...
    public void modifyPayStatus(String reservationId, String payStatus) {
        reservationReadMapper.updatePayStatus(reservationId, "Waiting");
    }
